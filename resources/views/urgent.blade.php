...
<head>
    ...
    <title>Laravel Todo App - Urgent Tasks</title>
...
<body class="bg-gray-100 p-4">
   ...
   class="px-4 py-2 text-gray-700 hover:bg-gray-200 rounded-xl font-bold">Urgent</a>
        </div>
    ...
    <div class="flex">
        ...
        <div class="w-1/6 bg-gray-200 p-4 rounded-xl">
            ...
            <ul id="taskGroups" class="space-y-2 font-bold">
                <li>
                    <a href="{{ route('todos.urgent') }}"
                        class="px-4 py-2 text-gray-700 hover:bg-gray-200 rounded-xl">
                        Urgent
                    </a>
                </li>
                @for ($i = 1; $i <= 4; $i++)
        ...
        <div class="flex-1 lg:w-4/5 p-4 overflow-auto">
            <div class="mb-4">
                <h2 class="font-bold text-2xl">Urgent Tasks</h2>
            </div>
            <!-- Urgent Task List Container with Limited Height -->
            <div class="flex flex-col h-screen overflow-hidden">
                <!-- Urgent Task List Content -->
                <div class="flex-1 overflow-y-scroll">
                    <div class="space-y-4">
                        @forelse ($urgentTasks as $urgentTask)
                            <div
                                class="flex items-center border-b border-gray-300 py-4 px-3 {{ $urgentTask->isDone ? 'bg-green-200' : '' }}">
                                <!-- Mark as Done Button -->
                                <form method="POST" action="{{ route('todos.markAsDone', $urgentTask->id) }}" class="mr-4">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <div class="flex-1 pr-8">
                                    <h3 class="text-lg font-semibold">{{ $urgentTask->title }}</h3>
                                    <p class="text-gray-500">{{ $urgentTask->description }}</p>
                                </div>

                                <!-- Urgent Task Action Buttons -->
                                <div class="flex space-x-3">
                                    <!-- Edit Urgent Task Button -->
                                    <button class="py-2 px-2 bg-blue-500 text-white rounded-xl"
                                        onclick="toggleEditForm({{ $urgentTask->id }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    ...
                                    <!-- Edit Urgent Task Form -->
                                    <div id="editForm-{{ $urgentTask->id }}" class="hidden">
                                    <form method="POST" action="{{ url('/todos/' . $urgentTask->id) }}">
                                            @csrf
                                            @method('PATCH')
                                            <input type="text" name="title" value="{{ $urgentTask->title }}"
                                                class="py-3 px-4 bg-gray-100 rounded-xl">
                                            <textarea name="description"
                                                class="py-3 px-4 bg-gray-100 rounded-xl">{{ $urgentTask->description }}</textarea>
                                            <button type="submit"
                                                class="w-28 py-4 px-8 bg-green-500 text-white rounded-xl">Update</button>
                                        </form>
                                    </div>

                                    <!-- Delete Urgent Task Button -->
                                    <form method="POST" action="{{ url('/todos/' . $urgentTask->id) }}">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <p class="text-gray-500">No urgent tasks found.</p>
                        @endforelse
                    </div>
                </div>
            </div>
