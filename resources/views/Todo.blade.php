        ...
        <div class="mt-2">
            ...
            <div class="py-4 flex items-center border-b border-gray-300 px-3 {{ $todo->isDone ? 'bg-green-200' : '' }}">
                ...
                <div class="flex space-x-3">
                    <!-- Edit button -->
                    <button class="py-2 px-2 bg-blue-500 text-white rounded-xl" onclick="document.getElementById('editForm-{{ $todo->id }}').classList.remove('hidden')">                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 20h9M16 4l-8 8-4-4v10h10l-4-4 8-8" />
                        </svg>
                    </button>

                    <!-- Edit form (hidden by default) -->
                    <form id="editForm-{{ $todo->id }}" class="hidden flex flex-col space-y-4" method="POST" action="{{ url('/todos/' . $todo->id . '/edit') }}">
                        @csrf
                        <input type="text" name="title" value="{{ $todo->title }}" class="py-3 px-4 bg-gray-100 rounded-xl">
                        <textarea name="description" class="py-3 px-4 bg-gray-100 rounded-xl">{{ $todo->description }}</textarea>
